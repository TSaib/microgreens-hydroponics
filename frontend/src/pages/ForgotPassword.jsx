...
export default function ForgotPassword() {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      setMsg(res.data.message || (res.data.success ? "Password reset link sent (demo)" : "Failed"));
    } catch (err) {
      setMsg("Server error");
    } finally {
      setLoading(false);
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(255,255,255,0.10)' }}>
      <form onSubmit={handleSubmit} style={{ background: 'rgba(255,255,255,0.97)', borderRadius: 18, boxShadow: '0 8px 32px rgba(0,0,0,0.13)', padding: '2.5rem 2.5rem 2rem 2.5rem', minWidth: 340, maxWidth: 380, width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', fontFamily: 'Inter, Arial, sans-serif' }}>
        <h2 style={{ color: '#23b758', fontFamily: 'Pacifico, cursive', fontWeight: 700, fontSize: '2.2rem', marginBottom: 18, letterSpacing: 1.2 }}>Forgot Password</h2>
        ...
        <button type="submit" disabled={loading} style={{ width: '100%', background: '#23b758', color: '#fff', border: 'none', borderRadius: 10, padding: '14px 0', fontWeight: 700, fontSize: '1.15rem', boxShadow: '0 2px 8px #23b75822', cursor: loading ? 'not-allowed' : 'pointer', marginTop: 6, marginBottom: 2, letterSpacing: 0.5 }}>{loading ? 'Sending...' : 'Send Reset Link'}</button>
        {msg && <div style={{ color: msg.includes('sent') ? '#188040' : '#b00', marginTop: 12 }}>{msg}</div>}
      </form>
    </div>
  );
}
